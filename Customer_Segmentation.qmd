...
My choice to create an CSV file before closing the analysis is because the current dataset "Segmented_data_Irvine.csv" can be used for Target Marketing. Utilizing the CustomerID section and Segement will allow the marketing team to isolate customers into their respective groups and contact them with promotions and rewards. 

```{r}
cleaned_data <- cleaned_clusters_sales |> 
  mutate(CustomerLifetimeValue = recency * frequency * monetary ) |> 
  mutate(rank_CLV = (CustomerLifetimeValue / sum(CustomerLifetimeValue) )* 100) |> 
  group_by(segment) |> 
  summarize(
    avg_recency = mean(recency),
    avg_frequency = mean(frequency),
    avg_monetary = mean(monetary),
    percent_of_lifetimevalue = sum(rank_CLV),
    total_value = sum(CustomerLifetimeValue)
  )
```
...
```
```{r}
ggplot(cleaned_data, aes(x = "", y = percent_of_lifetimevalue, fill = segment) ) +
...
```
Though the final calculation of customer lifetime value, and the relative 
percentages, we can see that 63.07% of customer lifetime value come from the 
high value customers, 16.48% come from occasional customers, 11.01% come
from Churning customers, and 9.43% come from inactive customers. 
